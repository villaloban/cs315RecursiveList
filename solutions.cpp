...
list reverseTopLevel(list p) {
    ...
    // use cdr of p because were doing the reverse first and then do the car since that should be last
...
bool equal(list p, list q) {
    //if there both empty return true;
    if (is_null(p) && is_null(q))
        return true;
    if (is_null(p) || is_null(q))
        return false;
...
//check for sublist instead of just atoms
bool member_equal(list p, list q) {
    if (is_null(q))
        return false;
    if (equal(p, car(q)))
        return true;
